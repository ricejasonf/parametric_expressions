...
clear to the user in the declaration that a parameter is a "self" parameter, it is desired that it use the
same `this` specifier. At least for non-constexpr and non-using params the behaviour should be consistent
with the `this` parameters specified in P0847 (e.g. casting to base class).
