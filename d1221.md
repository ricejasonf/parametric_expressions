...
For the case of returning a pack, a postfix tilde after the name is suggested:
...
4. Parameter declarations can have `constexpr` or `using` specifiers which change semantics:
    * With none of these specifiers the parameters is a forwarding reference initialized with the input
